...
반드시 한 문장으로 작성하세요.`
  ...
  const qnaPrompt = `당신은 네이버 카페 보험 Q&A SEO 최적화 전문가입니다.
...
20년 경력 베테랑 보험 전문가, 네이버 지식iN/카페 상위 0.1% 등급 '보험 분석 마스터'

### 답변 필수 구조 (4-Step Flow):
【 Step 1: 공감 시작 】
- 강렬한 한 줄 공감/경고 문장으로 시작
- 예: "😱 ${insuranceType}... 제가 봤을 때 **위험 신호**입니다."
- 예: "💔 그 마음 충분히 이해해요. 저도 처음엔 그랬거든요."
...
1. ${target} 시점에서 작성
2. 반드시 의문문으로 끝나야 함 (?)
3. ${hasKeyword ? `"${customerConcern}" 핵심 고민 반영` : `${insuranceType} 관련 고민 자동 생성`}
4. 클릭 유도 키워드 1개 이상 포함: 호구, 손해, 해지, 충격, 거절, 폭탄, 함정, 후회
5. 15-30자
- 예시: "${insuranceType} 이거 호구 잡힌 건가요?"
- 예시: "설계사가 해지하라는데 손해 보는 거 아닌가요?"

[질문]
★★★ ${hasKeyword ? `"${customerConcern}" 핵심 고민 중심!` : '상황 자동 생성'} ★★★
- ${target}의 말투로 작성 (보험 초보의 불안하고 답답한 심정)
- 인사 + 자기소개 (나이, 상황)
- 최근 겪은 일 (설계사 미팅, 유튜브 정보, 지인 조언)
- ${hasKeyword ? `핵심 고민: "${customerConcern}"` : `${insuranceType} 관련 일반적 고민 자동 생성`}
- 숫자 인용 가능 (월 XX만원, X년 납입 등)
- 마지막: "쪽지 사절이요, 댓글로 조언 부탁드립니다"
- 전화번호 절대 금지!
- 200-350자

[답변]
★★★ 4-Step Flow + 가독성 규칙 적용! ★★★
...
[댓글1]
공감형 - 40-80자, "저도 같은 고민이었는데", "속 시원하네요" 느낌

[댓글2]
사이다형 - 40-80자, "찐 정보", "바로 확인해봐야겠어요" 느낌

[댓글3]
질문형 - 40-80자, "저도 증권 분석 부탁드려도 될까요?" 느낌
...
- 예시: #${insuranceType} #${target}보험 #보험추천 #${coreKeywords[0]} #보험리모델링`
  ...
  // 제목 추출 (의문문 확인)
  let generatedTitle = titleMatch ? cleanText(titleMatch[1].trim()) : `${target} ${insuranceType} 이거 호구 잡힌 건가요?`
  // 의문문이 아니면 ? 추가
  ...
  // 질문 추출 (전화번호 제외, 1개)
  const mainQuestion = questionMatch 
    ? cleanText(questionMatch[1].trim()) 
    : `안녕하세요. ${target}인데 ${insuranceType} 관련해서 질문이 있어요. ${customerConcern} 쪽지 사절이요, 댓글로 조언 부탁드립니다.`
  
  // 호환성을 위해 questions 배열도 유지
  const questions = [mainQuestion].filter(q => q.length > 30)
  
  // 답변 추출 (1개, 가독성 규칙 적용된 형태)
  const mainAnswer = answerMatch 
    ? cleanText(answerMatch[1].trim()) 
    : `😱 ${insuranceType}... 제가 봤을 때 확인이 필요한 부분이 있습니다.\n\n${target}께서 걱정하시는 부분 충분히 이해합니다.\n\n전문가로서 딱 3가지만 짚어 드립니다.\n\n✅ **첫째:** 현재 상품의 보장 내역을 확인하세요.\n⚠️ **둘째:** 갱신형 특약이 있다면 주의가 필요합니다.\n💡 **셋째:** 증권을 사진 찍어 댓글로 남겨주시면 분석해 드립니다.`
  
  // 호환성을 위해 answers 배열도 유지  
  const answers = [mainAnswer].filter(a => a.length > 50)
