...
- Type B (호구 확인형): "${insuranceType} 가입했는데 저 호구 잡힌 건가요?"
- ⚠️ 절대 사연형("~한 사연") 금지! 반드시 의문문("~인가요?", "~할까요?")으로 끝내라!
- 반드시 "${insuranceType}"와 "${customerConcern}" 핵심 키워드 포함

■ 질문 3개 생성 (Question) - 완전히 다른 3명의 인물!
⚠️ 중요: "저도요", "저도 비슷해요" 금지! 각 질문은 완전히 다른 상황/배경/인물이어야 함!

- 질문1 (20대 사회초년생, 막 가입한 피해자):
  "저 ${target}인데요. 지인 추천으로 ${insuranceType} 가입했는데..."
  상황: 이미 가입 완료, 뒤늦게 문제 인식, 해지 고민 중
  말투: "ㅠㅠ", "눈탱이", "잠이 안 와요", 급박함

- 질문2 (40대 가장, 가입 전 신중 검토 중):
  "안녕하세요. ${insuranceType} 가입 전에 전문가분들 의견 듣고 싶습니다."
  상황: 아직 미가입, 설계사 제안 받은 상태, 팩트 체크 중
  말투: "이게 맞나요?", "실제로 어떤가요?", 이성적/신중함

- 질문3 (50대, 기존 보험 리모델링 고민):
  "10년 전에 가입한 ${insuranceType}이 있는데 갈아타야 할지 고민입니다."
  상황: 구형 보험 보유, 갱신료 폭등 또는 보장 부족 인식
  말투: "비교해주세요", "어떤 게 나을까요?", 비교 분석 요청

■ 전문가 답변 3개 생성 (Answer) - 네이버 팩트 기반 디테일 분석!
⚠️ 중요: 단순 상담 멘트 금지! 반드시 구체적 수치/약관/데이터로 분석!

- 답변1 (팩트 폭격 전문가):
  "결론부터 말씀드립니다. [구체적 결론]"
  내용: 2026년 약관 기준, 사업비율(%), 예정이율(%), 해지환급금 예시
  예시: "납입금의 30%가 사업비로 빠집니다. 2026년 예정이율 2.5% 기준, 원금 도달까지 7년 걸립니다."
  
- 답변2 (비교 분석 전문가):
  "[현황 분석] → [문제점] → [대안 비교]"
  내용: 갱신형 vs 비갱신형 보험료 비교, 회사별 상품 비교, 특약 비교
  예시: "A사 갱신형: 월 3만원→65세 9만원 / B사 비갱신형: 월 5만원 고정. 20년 총 납입액 비교하면..."

- 답변3 (솔루션 제시 전문가):
  "세 가지 선택지를 드립니다."
  내용: 1) 유지 시 장단점 2) 해지 시 손실액 계산 3) 감액완납/전환 등 대안
  예시: "해지하면 150만원 손해, 감액완납하면 원금의 70% 보존, 전환하면..."

■ 댓글 5개 생성 (Comment) - 각각 다른 사람!
- 댓글1: 피해 경험자 공감 ("저도 당했어요... 2년 붓고 400만원 날림")
- 댓글2: 정보 감사 ("${insuranceType} 정보 감사합니다. 바로 증권 확인해봐야겠네요")
- 댓글3: 다른 상황 질문 ("저는 ${insuranceType} 3년차인데 이 경우는 어떤가요?")
- 댓글4: 성공 경험담 ("저 리모델링하고 보험료 월 5만원 아꼈어요")
- 댓글5: 응원 + 액션 ("화이팅! 증권 분석 받아보세요. 저도 그렇게 해결했어요")
...
⚠️ [태그]와 내용만 출력! 설명, 구분선, 마크다운(**, ##) 출력 금지!`
  ...
  try {
    ...
    // V17.3 리얼 토크 스타일 (사이다) - 완전히 다른 3명
    if (isRealTalk) {
      return `[제목1]
${target}인데 ${insuranceType} 이거 어떻게 해야 하나요?

[제목2]
${insuranceType} 가입했는데 저 호구 잡힌 건가요?

[질문1]
형님들 급질문요 ㅠㅠ 저 20대 사회초년생인데요.
지난달에 회사 선배가 ${insuranceType} 좋다고 해서 월 30만원짜리 사인했어요.
${customerConcern}
...
[질문3]
50대 자영업자입니다.
10년 전에 가입한 ${insuranceType}이 있는데 요즘 고민이 많습니다.
${customerConcern}
...
${customerConcern} 상황이시라면 지금 당장 증권 펴세요.
${insuranceType}의 진실을 팩트로 알려드리겠습니다.

【 핵심 수치 분석 】
• 사업비율: 납입금의 약 25~35%가 수수료로 빠집니다
• 예정이율: 2026년 기준 2.25~2.5% (은행 예금 3%보다 낮음)
• 원금 도달: 최소 7~10년 소요 (중도해지 시 원금의 50~70%만 환급)
• 해지환급금: 3년차 기준 납입액 대비 약 40~60% 수준

【 결론 】
지금 해지하면 손해지만, 10년 더 유지해도 은행 예금보다 수익 낮습니다.
댓글로 정확한 상품명 알려주시면 탈출 플랜 짜드립니다.

[답변2]
20년차 보험 분석 전문가입니다. 비교 분석 들어갑니다.

【 ${insuranceType} 갱신형 vs 비갱신형 비교 】

▶ 갱신형 (현재 많이 파는 상품)
- 초기 보험료: 월 3만원 (30대 기준)
- 65세 예상: 월 9~12만원 (3~4배 인상)
- 20년 총 납입액: 약 1,440만원 → 2,880만원

▶ 비갱신형 (추천)
- 초기 보험료: 월 5만원 (30대 기준)
- 65세까지: 월 5만원 (인상 없음)
- 20년 총 납입액: 1,200만원 고정

【 결론 】
갱신형은 65세 이후 '보험료 폭탄' 맞습니다.
${customerConcern} 상황이시라면 비갱신형 전환을 검토하세요.

[답변3]
세 가지 선택지 드립니다. 숫자로 보여드릴게요.

【 선택지 1: 해지 】
- 현재 환급금: 납입액의 약 50~60% (예: 100만원 납입 → 50~60만원 환급)
- 장점: 즉시 현금 확보, 더 이상 손해 없음
- 단점: 지금까지 납입한 금액 일부 손실

【 선택지 2: 감액완납 】
- 납입 중단하고 보험 유지 (보장금액만 줄어듦)
- 장점: 원금의 70~80% 보존, 최소한의 보장 유지
- 단점: 보장 금액 대폭 축소

【 선택지 3: 유지 】
- 7~10년 더 유지해야 원금 도달
- 장점: 손실 없음 (시간이 해결)
- 단점: 기회비용 손실, 더 좋은 상품 가입 불가

${customerConcern} 상황에서 제 추천은 '감액완납 후 비갱신형 신규 가입'입니다.
증권 사진 주시면 정확한 계산해드립니다.

[댓글1]
저 2년 붓고 400만원 날리고 해지했어요... 하루라도 빨리 나오는 게 돈 버는 겁니다

[댓글2]
${insuranceType} 정보 감사합니다. 저도 바로 증권 확인해봐야겠네요

[댓글3]
저는 ${insuranceType} 5년차인데 이 경우는 감액완납이 답인가요? 상황이 좀 달라서요

[댓글4]
저 작년에 리모델링하고 월 5만원 아꼈어요. 비갱신형으로 바꾸길 잘했습니다

[댓글5]
화이팅! 전문가 분석 받아보세요. 저도 그렇게 해결했어요
...
[질문3]
안녕하십니까. 50대 자영업자입니다.
10년 전 가입한 ${insuranceType}의 리모델링을 고민 중입니다.
${customerConcern}
...
【출력 형식 - 반드시 JSON만 출력】
{
  ...
  return c.json({
    ...
    // 버전 정보
    version: 'V17.3-MultiPersona-QualityUp'
  })
})
