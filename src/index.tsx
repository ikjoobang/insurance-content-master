...
async function generateContentWithStrategy(
  ...
  // ============================================================
  // V18.3: 답변에서 연령대만 사용 (직업 호칭 제거)
  // "50대 조기퇴직자님" → "50대님" 또는 "회원님"
  // ============================================================
  const ageMatch = target.match(/(\d+)대/)
  const targetAge = ageMatch ? ageMatch[0] : '회원'  // "50대" 또는 "회원"
  const targetNickname = ageMatch ? `${ageMatch[0]}님` : '회원님'  // "50대님" 또는 "회원님"
...
- "화이팅하세요! 좋은 결과 있으시길 바랍니다 :)"`
  ...
  // 시나리오 데이터베이스에서 해당 보험 유형의 패턴 가져오기
  const scenario = getRandomScenario(insuranceType)
  const scenarioGuide = scenario ? `
【 📌 참고: 실제 네이버 카페 질문 패턴 예시 】
유형: ${scenario.type}
질문 예시:
${scenario.questionTemplates.map((t, i) => `${i+1}. "${t}"`).join('\n')}

전문가 답변 패턴:
- 인사: "${scenario.expertAnswerPattern.greeting}"
- 분석: "${scenario.expertAnswerPattern.analysis}"
- 체크포인트:
${scenario.expertAnswerPattern.checkpoints.join('\n')}
- CTA: "${scenario.expertAnswerPattern.cta}"
` : ''
  
  const contentPrompt = `########################################################################
# System Role: High-Fidelity Insurance Persona Generator (V17.5 RealPattern)
########################################################################
...
⚠️⚠️⚠️ 위 사진 분석 결과를 Q&A에 반드시 반영하세요! ⚠️⚠️⚠️

【 질문 작성 시 】
- 사진 속 "보험사명", "상품명"을 질문에 언급 (예: "삼성생명 암보험 가입했는데...")
- "계약일"이 오래됐으면 "○○년 전에 가입한 보험인데..." 형식
- "월 보험료" 금액을 구체적으로 언급 (예: "월 12만원짜리...")

【 전문가 답변 작성 시 - 핵심! 】
- 사진 속 "보험사명"과 "상품명"을 언급하며 분석
- "월 보험료", "납입기간", "보험기간"을 구체적 수치로 언급
- "특약목록"의 각 특약명과 금액을 하나하나 언급하며 분석
  예: "뇌출혈진단 2,000만원 특약이 갱신형이네요. 10년 후 보험료가..."
- "갱신형/비갱신형" 비율을 언급하며 향후 보험료 상승 경고
- "핵심 문제점"을 답변의 핵심 분석에 포함
- "전문가 조언"의 내용을 적극 활용하여 구체적인 해결책 제시

【 금지 사항 】
- 사진에 없는 보험사/상품명 언급 금지
- 사진의 금액과 다른 금액 언급 금지
- 일반적인 뻔한 답변 금지 - 반드시 이 사진의 데이터 기반으로!
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
` : ''}
...
【 생성 가이드 V17.5 - 네이버 카페 리얼 패턴 】

⚠️ 핵심 원칙: 모든 질문자는 "${target}"이어야 함! 다른 연령대 언급 금지!

■ 제목 2개 생성 (Title) - 반드시 "?" 물음표로 끝나는 질문형!
- Type A (구체적 상황형): "월 {금액}원짜리 ${insuranceType} 이거 괜찮을까요?"
- Type B (발견/점검형): "장롱 속에 묻혀있던 ${insuranceType} 발견했는데 이거 리모델링해야 하죠?"
- Type C (추천받음형): "${insuranceType} 추천받았는데 한번 봐주세요"
- ⚠️ 절대 사연형("~한 사연") 금지! 반드시 의문문("~인가요?", "~할까요?", "~주세요")으로 끝내라!
- ⚠️ 구체적 금액/상황을 제목에 포함하라! (월 3만원, 2007년 가입, 메리츠 등)

■ 질문 3개 생성 (Question) - 모두 "${target}"! 구체적 상황 필수!
⚠️ 핵심: 3명 모두 "${target}"! 상황/상품/금액만 다르게!
⚠️ "저도요", "저도 비슷해요" 금지! 각 질문은 완전히 다른 구체적 상황이어야 함!

- 질문1 (${target}, 지인/설계사 권유로 최근 가입):
  구체적 상황 필수: "월 {금액}원짜리 {보험사명} ${insuranceType} 가입했는데..."
  포함 요소:
    • 구체적 금액 (월 17만원, 월 25만원 등)
    • 가입 경위 (엄마 친구 설계사, 회사 동료, 유튜브 보고)
    • 현재 고민 ("${customerConcern}")
    • 감정 표현 (불안, 의심, 후회)
  말투: 급박하고 절실한 톤

- 질문2 (${target}, 오래된 증권 발견/점검):
  구체적 상황 필수: "{년도}년에 가입한 {보험사명} ${insuranceType} 서랍에서 발견했어요"
  포함 요소:
    • 구체적 년도 (2007년, 2015년, 10년 전)
    • 보험사명 (한화, 삼성, 메리츠 등)
    • 현재 고민 ("${customerConcern}")
    • 점검/리모델링 니즈
  말투: 걱정되는 톤, 전문가 확인 요청
...
- 질문3 (${target}, 가입 전 비교/검토 중):
  구체적 상황 필수: "{보험사명} ${insuranceType} 월 {금액}원 제안받았는데 이거 괜찮은가요?"
  포함 요소:
    • 설계사 제안 내용 (월 보험료, 보장 내용)
    • 팩트 체크 요청
    • 현재 고민 ("${customerConcern}")
    • 비교 대상 언급
  말투: 신중하고 분석적인 톤
...
■ 전문가 답변 3개 생성 (Answer) - 인사+칭찬 → 핵심결론 → 구체분석 → 체크포인트!
⚠️ 핵심 구조: 【인사+칭찬】→【핵심 결론】→【구체적 분석】→【체크포인트】→【CTA】
⚠️ 로봇처럼 수치만 나열하지 마! 먼저 질문자를 칭찬하고 공감하라!

- 답변1 (인사+분석형 전문가):
  【인사+칭찬】"좋은 질문입니다! ${targetNickname}, 이렇게 꼼꼼히 확인하시려는 거 정말 현명하세요."
  【핵심 결론】"결론부터 말씀드리면요..." (2-3문장으로 핵심)
  【구체적 분석】"${insuranceType}을 자세히 살펴보면..." (팩트 기반 설명)
    • 2026년 기준 사업비율 25~35%
    • 예정이율 2.25~2.5% (은행 예금 3% 대비 낮음)
    • 해지환급금 예시 (3년차: 납입액의 40~60%)
  【체크포인트】"다만 체크해봐야 할 부분도 분명해요"
    ✔️ 첫 번째 체크포인트
    ✔️ 두 번째 체크포인트
    ✔️ 세 번째 체크포인트
  【CTA】"증권 사진 올려주시면 구체적으로 분석해드릴게요!"
  
- 답변2 (경험담+비교형 전문가):
  【인사+경험】"안녕하세요! ${targetNickname}과 비슷한 분들 상담 정말 많이 해봤어요."
  【사례 공유】"얼마 전에 비슷한 케이스가 있었는데요..." (구체적 사례)
  【비교 분석】"갱신형 vs 비갱신형, 쉽게 비교해드리면..."
    • 갱신형: 지금 월 3만원 → 65세에 12만원 (3~4배 폭등)
    • 비갱신형: 지금 월 5만원 → 평생 5만원 고정
    • 20년 총 납입 비교: 갱신형 2,800만원 vs 비갱신형 1,200만원
  【조언】"${targetNickname} 상황에서는 이게 더 맞을 것 같아요"
  【CTA】"궁금한 거 있으시면 편하게 댓글 주세요!"

- 답변3 (솔루션 제시형 전문가):
  【인사+공감】"${targetNickname}, 고민 많으시죠ㅠㅠ 내용 잘 정리해서 올려주셨어요."
  【선택지 정리】"선택지를 깔끔하게 정리해드릴게요"
    【선택지 1: 해지】→ 장점/단점/예상 환급금
    【선택지 2: 감액완납】→ 장점/단점/보존 가능 금액
    【선택지 3: 유지】→ 장점/단점/손익분기점
  【추천】"제 생각엔 ${targetNickname} 상황에서는 '감액완납'이 제일 나을 것 같아요"
  【근거】"왜냐하면... (구체적 이유)"
  【CTA】"자세한 건 증권 보고 계산해드릴게요. 언제든 물어보세요!"

■ 댓글 5개 생성 (Comment) - 모두 ${target} 또는 비슷한 연령대!
- 댓글1: 공감 ("저도 ${target}인데 똑같은 고민이에요 ㅠㅠ 여기 댓글 보고 용기 얻고 갑니다")
- 댓글2: 감사+행동 ("${insuranceType} 정보 감사합니다. 저도 바로 증권 확인해봐야겠네요")
- 댓글3: 구체적 추가 질문 ("저도 ${target}인데요, 2018년에 가입한 거는 어떻게 해야 할까요?")
- 댓글4: 성공 경험담 ("저 작년에 리모델링하고 월 5만원 아꼈어요. 비갱신형으로 바꾸길 잘했습니다!")
- 댓글5: 응원 ("화이팅! 전문가 분석 받으면 해결돼요. 저도 그렇게 했어요 :)")
...
[제목1]
(구체적 금액/상황 포함 질문형! 예: "월 17만원짜리 ${insuranceType} 이거 괜찮을까요?")

[제목2]
(발견/점검형 질문! 예: "2015년에 가입한 ${insuranceType} 리모델링해야 하죠?")

[질문1]
(${target}, 지인/설계사 권유로 최근 가입, 구체적 금액/보험사명 포함, "${customerConcern}" 포함, 300자 이상)

[질문2]
(${target}, 오래된 증권 발견/점검, 구체적 년도/보험사명 포함, "${customerConcern}" 포함, 300자 이상)
⚠️ "저도요" 금지! 질문1과 완전히 다른 구체적 상황!

[질문3]
(${target}, 가입 전 비교/검토 중, 설계사 제안 내용 포함, "${customerConcern}" 포함, 300자 이상)
⚠️ "저도요" 금지! 질문1,2와 완전히 다른 구체적 상황!

[답변1]
(인사+칭찬 → 핵심결론 → 구체분석(사업비율%, 예정이율%, 해지환급금) → 체크포인트 → CTA, 500자 이상)

[답변2]
(인사+경험담 → 사례공유 → 갱신형vs비갱신형 비교(구체적 금액) → 조언 → CTA, 500자 이상)

[답변3]
(인사+공감 → 선택지3개(해지/감액완납/유지 각각 장단점) → 추천+근거 → CTA, 500자 이상)

[댓글1]
(공감 댓글, 구체적 상황 언급, 40-100자)
...
${insuranceType} 때문에 밤잠 설치시는 분들 정말 많이 봤는데, 대부분 해결됐어요!
