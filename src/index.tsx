...
- "화이팅하세요! 좋은 결과 있으시길 바랍니다 :)"`
  ...
  // ============================================================
  // 🔥 V17.1 클린 프롬프트 - 예시 베끼기 차단 버전
  // ============================================================
  
  const contentPrompt = `########################################################################
# System Role: High-Fidelity Insurance Persona Generator (V17.1 Clean)
########################################################################
...
■ 제목 2개 생성 (Title)
- Type A (감정형): "${target}이 ${insuranceType} 가입했다가... 저 지금 호구 잡힌 건가요?"
- Type B (직설형): "${insuranceType}, 설계사 말 믿고 사인하면 안 되는 결정적 이유"
- 반드시 "${insuranceType}"와 "${customerConcern}" 핵심 키워드 포함

■ 질문 3개 생성 (Question) - 서로 다른 인격
- Persona 1 (다급한 피해자): 이미 가입했고 뒤늦게 잘못된 걸 알게 됨. "ㅠㅠ", "눈탱이", "잠이 안 와요" 감정적 말투
- Persona 2 (깐깐한 의심러): 가입 직전, 팩트 체크 중. "이게 맞나요?", "팩트 부탁드립니다" 이성적 말투
- Persona 3 (정보 탐색자): 비교 분석 중. "어떤 게 나을까요?" 중립적 말투
- 각 질문에 "${customerConcern}"과 "${insuranceType}" 필수 포함

■ 전문가 답변 3개 생성 (Answer) - 서로 다른 전문가
- Expert A (독설가): "잠깐만요! 그 펜 내려놓으세요." 강력 경고 스타일
- Expert B (분석가): "2026년 약관 기준으로 분석해 드립니다." 수치/근거 스타일
- Expert C (대안 제시자): "해지보다는 '감액 완납' 제도를 활용하세요." 부드러운 스타일
- 각 답변에 "${insuranceType}" 최소 3회 언급, "${customerConcern}" 반드시 포함

■ 댓글 5개 생성 (Comment)
- 댓글1: "${customerConcern}" 공감 (피해자/경험자)
- 댓글2: 전문가 답변에 감사, "${insuranceType}" 언급
- 댓글3: 비슷한 상황의 추가 질문
- 댓글4: 성공담/해결 경험 공유
- 댓글5: 질문자 응원 또는 쪽지 요청
...
[제목1]
(${target}의 ${insuranceType} 관련 어그로성 제목, 의문문)

[제목2]
(${insuranceType} 관련 정보성 제목)

[질문1]
(다급한 피해자 스타일, "${customerConcern}" 포함, 300자 이상)

[질문2]
(깐깐한 의심러 스타일, "${customerConcern}" 포함, 300자 이상)

[질문3]
(정보 탐색자 스타일, "${customerConcern}" 포함, 300자 이상)

[답변1]
(독설가 전문가, "${insuranceType}" 3회+, "${customerConcern}" 포함, 500자 이상)

[답변2]
(분석가 전문가, "${insuranceType}" 3회+, "${customerConcern}" 포함, 500자 이상)

[답변3]
(대안 제시 전문가, "${insuranceType}" 3회+, "${customerConcern}" 포함, 500자 이상)
...
[댓글2]
보험 닥터한테 진단받고 싹 갈아엎음. 속이 다 시원함 ㅋㅋ
...
` : ''
    ...
    console.log(`[V17.1 RAG Step 3] ${currentAttempt}차 콘텐츠 생성 시작... (톤: ${tone})`)
...
【출력 형식 - 반드시 JSON만 출력】
{
  ...
  return c.json({
    ...
    // 버전 정보
    version: 'V17.1-CleanPrompt-HumanLike'
  })
})
